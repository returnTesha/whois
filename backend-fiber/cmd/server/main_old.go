...
import (
	...
	"github.com/returnTesha/whois/internal"
)
...
func main() {
	...
	drawingService := internal.NewDrawingService("http://spring-service/api/spring/v1")
	...
	app.Post("/visit", func(c *fiber.Ctx) error {
		log.Printf("X-Forwarded-For: %s", c.Get("X-Forwarded-For"))
		log.Printf("X-Real-IP: %s", c.Get("X-Real-IP"))
		log.Printf("c.IP(): %s", c.IP())
